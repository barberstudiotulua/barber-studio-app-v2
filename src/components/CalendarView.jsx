...
// Este componente recibe la duración total que necesita buscar
function CalendarView({ totalServiceDuration, onSlotSelect, selectedTimeSlot }) {
  ...
  const fetchAvailableSlots = useCallback(async (selectedDate, durationNeeded) => {
    // Si no hay duración (ej: no se han seleccionado servicios), no hacemos nada.
    ...
    // Llamamos a la función correcta en Supabase, pasándole la duración total que ya calculamos
    ...
    if (error) {
      console.error('Error al buscar horarios:', error.message);
  ...
  useEffect(() => {
    // Este efecto llama a la función de búsqueda cada vez que cambia la fecha o la duración total
